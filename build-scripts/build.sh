...
# https://uvasgd.github.io/sgd-docs/unity/continuous-integration.html
...
/Applications/Unity/Unity.app/Contents/MacOS/Unity -batchmode -nographics	-silent-crashes	-logFile $(pwd)/unity.log	-projectPath "$(pwd)/${UNITYCI_PROJECT_NAME}"	-buildWindowsPlayer "$(pwd)/Build/windows/${UNITYCI_PROJECT_NAME}.exe" -quit
...
/Applications/Unity/Unity.app/Contents/MacOS/Unity -batchmode	-nographics -silent-crashes -logFile $(pwd)/unity.log -projectPath "$(pwd)/${UNITYCI_PROJECT_NAME}" -buildOSXUniversalPlayer "$(pwd)/Build/osx/${UNITYCI_PROJECT_NAME}.app" -quit
